...
def lvl1_render(prd_id: int, front_cached: bool, user_id: int):
    ...
    return requests.post(
        f'{bg_link}/public/bg_tasks/ext_prd/lvl1', json={'prd_id': prd_id, 'front_cached': front_cached, 'user_id': user_id}).json()

@celery_bg.task(queue=ext_prd_queue, routing_key=large_prd_routing_key)
def lvl2_render(prd_id: int, seller_id: int):
    log_event(Events.bg_queue_enter.format(2) + f'prd_id = {prd_id}', level='INFO')
    return requests.post(
        f'{bg_link}/public/bg_tasks/ext_prd/lvl2', json={'prd_id': prd_id, 'seller_id': seller_id}).json()

@celery_bg.task(queue=ext_prd_queue, routing_key=large_prd_routing_key)
def lvl3_render(prd_id: int):
    log_event(Events.bg_queue_enter.format(3) + f'prd_id = {prd_id}', level='INFO')
    return requests.post(
        f'{bg_link}/public/bg_tasks/ext_prd/lvl3', json={'prd_id': prd_id}).json()
...
def sending_email_code(email: EmailStr, user: Record, reset_token: str):
    ...
    return requests.post(
        f'{bg_link}/public/bg_tasks/email_check', json={'email': email,'user': user,'reset_token': reset_token})
...
def bg_s3_upload(self, file_path: str):
    try:
        response = requests.put(f'{bg_link}/private/bg_tasks/s3_upload/saving', params={'file_path': file_path})
...
@celery_bg.task()
def run_rT_cleaner():
    requests.delete(f'{bg_link}/server/crons/flush_refresh-tokens').json()

@celery_bg.task()
def run_messages_cleaner():
    requests.delete(f'{bg_link}/server/crons/delete_chat-messages').json()

@celery_bg.task()
def transfer_to_s3():
    requests.put(f'{bg_link}/server/crons/s3_fs-manager').json()
