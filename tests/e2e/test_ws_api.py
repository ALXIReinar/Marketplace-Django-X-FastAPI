...
class TestWS:
    ...
    async def test_chats_layout(self, ac):
        await ac.put('/api/chats/commit_msg', json={'event': 'commit_msg', 'chat_id': 1, 'msg_id': 4})
    ...
    async def test_upload_local_file(self, ac):
        ...
        async with websockets.connect(ws_link) as ws:
            ...
            assert res['success'] == True and res['msg_id'] == 5
            assert Path(f'.{env.local_storage}/users/chats/{res["file_name"]}.png').exists() == True
